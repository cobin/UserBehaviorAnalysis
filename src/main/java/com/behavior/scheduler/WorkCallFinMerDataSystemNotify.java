...
public class WorkCallFinMerDataSystemNotify extends WorkJob {
	...
	public void execWork(BehaviorMain bm,String sdate ){
		...
		//所有的财务数据
		Map<Integer,Map<Integer,Map<String,Object>>> query = getFinMerAll(ct9104);
		//所有的已经存在的流水记录
		Map<String,Map<String,Object>> exitFlowDetail = getFowDetails(ct9104);
		List<Map<String,Integer>> matchFlows = new ArrayList<>();
		Map<String,String> matchExist = new HashMap<>();
		//遍历访问此流水号是否有父类
		for(Map<String,Integer> id:ids){
			int rId = id.get("rId");
			searchFinMerId(matchFlows, matchExist, exitFlowDetail, query, rId, new int[]{rId});
		}
		log.info("财务流水处理,耗时:"+(System.currentTimeMillis()-curTime)+">>>>总数："+matchExist.size()+"("+matchFlows.size()+")");
	 ...
	 */
	private void searchFinMerId(List<Map<String,Integer>> matchFlows ,Map<String,String> matchExist,Map<String,Map<String,Object>> exitFlowDetail,Map<Integer,Map<Integer,Map<String,Object>>> query,int rId,int[] curRids){
		...
		for(Map.Entry<Integer,Map<String,Object>> me:row.entrySet()){
			...
			if(isNotExit){
				...
				for(int curRid:curRids) {
					addMatchFlow(matchFlows, matchExist,exitFlowDetail,mv, curRid, _mId);
					_curRids[i] = curRid;
					i++;
				}
				_curRids[i] = _mId;
				searchFinMerId(matchFlows,matchExist,exitFlowDetail,query,_mId,_curRids);
			}else {
				addMatchFlow(matchFlows, matchExist,exitFlowDetail,mv, rId, _mId);
			}
		}
	}
	 ...
	 * @param _mId
	 */
	private void addMatchFlow(List<Map<String, Integer>> matchFlows, Map<String, String> matchExist,Map<String,Map<String,Object>> exitFlowDetail,Map<String,Object> curRowData, int rId, int _mId) {
	 ...
	 * @return
	 */
	private Map<Integer,Map<Integer,Map<String,Object>>> getFinMerAll(CallTask9104Mapper ct9104){
		List<Map<String,Object>> result = ct9104.getFinMerDataSystemAll();
